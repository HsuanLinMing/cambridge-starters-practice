...
- 🟡 P3-10-V：正式題庫 `sourceProvenance` schema + approve 轉換保留——**部分完成**（2026-07-07）：`lib/types.ts` 新增 optional `QuestionSourceProvenance` / `BaseQuestion.sourceProvenance`；`scripts/approve_reviewed_questions.mjs` v0.1.1 → v0.1.2，在 reviewed item 有 `sourceUrl` 時把 sourceId / sourceUrl / documentTitle / pageHint / sectionHint / sourceKind / publisher / publisherType / rightsNotes / provenanceNotes / reviewerNotes 保留到正式 `ExamQuestion` preview / `/tmp` write target。`question.source` 仍只接受 `QuestionSource` union 4 種，source registry 細分類（如 `official_learning_material`）保留於 `sourceProvenance.sourceKind`。**未做**：正式題庫寫入 / 正式 paper 寫入 / `/quiz` 載入切換；本輪只補來源追溯 schema 與 approve 轉換。
...
- **v1.8**（2026-07-07）：新增 P3-10-V 條目——正式題庫 `sourceProvenance` schema + approve 轉換保留；`lib/types.ts` 補 optional source provenance 型別，`approve_reviewed_questions.mjs` v0.1.2 在 preview / `/tmp` write target 保留來源追溯。正式題庫 / 正式 paper / `/quiz` 仍未切換，P3-10 / P3 整體仍進行中。
