...
| `spelling` | **RW3**（P3-9-C 第三刀後續） | 🟢 較接近（最直接對齊看圖拼字輸入） | 圖 + 提示語 + 大型輸入框已實作；normalize 比對忽略大小寫與前後空白；**已支援 4 題多元主題**（apple / cat / dog / book，覆蓋 food / animals / school 三大類）；缺字提示版（部分字母 + 底線）屬未來進階；review 區獨立練習模式整合屬 P2-4C-2B-2 |
