...
- **未來需要補哪些功能**：
  - ~~多題 RW3 spelling 題庫~~ ✅ **2026-05-10 補件完成**——目前 4 題（apple / cat / dog / book，覆蓋 food / animals × 2 / school 三大類）。
  - ~~拼字提示渲染（部分字母 + 底線）~~ ✅ **2026-05-12 第一版完成**——schema 加 optional `spellingHint?: string`、UI 在 SpellingView 條件渲染淡藍提示區塊；4 題 q-sp-001~004 皆已補 hint（`a _ _ l e` / `c _ t` / `d _ g` / `b _ _ k`）；**僅 UI 提示、不參與 `isCorrect`**。
  - ~~字母重組版（例如 `eppal` 打散字母拼回 `apple`）~~ ✅ **2026-05-13 第一版完成**——schema 加 optional `letterScramble?: string`、UI 在 SpellingView 緊接 spellingHint 之後條件渲染淡紫提示區塊；4 題 q-sp-001~004 皆已補 scramble（`p p a l e` / `t a c` / `g d o` / `o b k o`）；**僅 UI 顯示打散字母、不參與 `isCorrect`、第一版不做拖曳 / 點選組字互動**（未來進階可加）。
...
| `spelling` | **RW3**（P3-9-C 第三刀後續） | 🟢 較接近（最直接對齊看圖拼字輸入） | 圖 + 提示語 + 大型輸入框已實作；normalize 比對忽略大小寫與前後空白；**已支援 4 題多元主題**（apple / cat / dog / book，覆蓋 food / animals / school 三大類）；**缺字提示版（optional `spellingHint`）已第一版落地**（2026-05-12）；**字母重組版（optional `letterScramble`）已第一版落地**（2026-05-13，4 題皆補 scramble，僅 UI 提示、不參與 isCorrect、第一版不做拖曳 / 點選組字）；review 區獨立練習模式整合屬 P2-4C-2B-2 |
...
- **v2.8**（2026-05-13，P3-9-C 第三刀後續 RW3 字母重組版第一版）：在 v2.7 缺字提示版基礎上補 optional `letterScramble?: string` 欄位 + UI 條件渲染。`lib/types.ts` `SpellingQuestion` 加 optional `letterScramble?: string`（含完整 docstring 標「**只用於 UI 顯示提示、絕不參與 `answer` 比對**——`isCorrect` 不讀此欄位、`normalize` 不變、第一版不做拖曳 / 點選組字互動」）；`components/QuizPlay.tsx` `<SpellingView>` 在 spellingHint 區塊之後 / 輸入框之前條件渲染淡紫提示區塊（`bg-violet-50` 與 spellingHint 的 `bg-sky-50` 視覺區別、標籤「字母重組」+ 同 font-mono text-3xl/4xl + tracking-[0.4em] 大字間距、僅 question.letterScramble 存在時顯示）；`isCorrect` / `normalize` / `formatUserAnswer` / `formatCorrectAnswer` / `getStarterPartInfo` / `RW_TYPE_ORDER` / 結果頁 / Listening / RW1 yes/no / L3 圖卡完全不動；既有 `spellingHint` 區塊保留並先於 letterScramble 顯示——一題可同時顯示「缺字提示」+「字母重組」兩種視覺輔助。`data/p3-example-questions.json` 4 題 q-sp-001~004 皆補 `letterScramble`（apple→`p p a l e` / cat→`t a c` / dog→`g d o` / book→`o b k o`），其他欄位（含既有 `spellingHint`）全保留；未新增題目 / 圖片 / 音檔。RW3 模板未來功能清單把「字母重組版」標 ✅；schema 對應表 spelling 條目同步「字母重組版已第一版落地」；DATA_SCHEMA spelling 段 jsonc 範例 + 要點補 letterScramble 說明 + UI 渲染段補淡紫區塊描述。**未動** isCorrect 比對行為（仍 normalize trim + toLowerCase、仍不做 fuzzy matching）/ 題目 / 答案 / spellingHint / Listening / RW1 yes/no / L3 圖卡 / 圖片 / 音檔 / 結果頁邏輯。**仍未做**：拖曳字母互動 / 點選字母組字互動 / 看答案 / 再試一次按鈕 / 與 review 區獨立練習模式整合。**硬邊界不變**。
- **v2.7**（2026-05-12，P3-9-C 第三刀後續 RW3 缺字提示版第一版）：在 v2.6 多題題庫基礎上補 optional `spellingHint?: string` 欄位 + UI 條件渲染。`lib/types.ts` `SpellingQuestion` 加 optional `spellingHint?: string`（含完整 docstring 標「**只用於 UI 顯示提示、絕不參與 `answer` 比對**——`isCorrect` 不讀此欄位」）；`components/QuizPlay.tsx` `<SpellingView>` 在圖 + 提示語之後 / 輸入框之前條件渲染淡藍提示區塊（標籤「缺字提示」+ font-mono text-3xl/4xl + tracking-[0.4em] 大字間距，sky-50 背景區別於圖片區）；`isCorrect` / `normalize` / `formatUserAnswer` / `formatCorrectAnswer` 完全不變。`data/p3-example-questions.json` 4 題 q-sp-001~004 皆補 spellingHint（`a _ _ l e` / `c _ t` / `d _ g` / `b _ _ k`），其他欄位 image / prompt / answer / explanation / metadata 全保留。RW3 模板未來功能清單把「拼字提示渲染」標 ✅；schema 對應表 spelling 條目同步「缺字提示版已第一版落地」；DATA_SCHEMA spelling 段 jsonc 範例 + 要點補 spellingHint 說明。**未動** isCorrect 比對行為（仍 normalize trim + toLowerCase、仍不做 fuzzy matching）/ 題目 / 答案 / Listening / RW1 yes/no / L3 圖卡 / 圖片 / 音檔 / 結果頁邏輯。**仍未做**：字母重組版 / 看答案 / 再試一次按鈕 / 與 review 區獨立練習模式整合。**硬邊界不變**。
- **v2.6**（2026-05-10，P3-9-C 第三刀後續 RW3 多題拼字題庫第一版）：在 v2.5 spelling 題型基礎上補 3 題 RW3 spelling（q-sp-002 cat / q-sp-003 dog / q-sp-004 book），讓 RW3 不再只有 apple 一題；目前 4 題覆蓋 food（apple）/ animals（cat / dog）/ school（book）三大主題。`data/p3-example-questions.json` 加 3 題（同 q-sp-001 結構：source ai_generated / 重用既有 SVG cat.svg / dog.svg / book.svg / prompt 「Look at the picture. Write the word.」/ explanation 中文 / RW3 metadata 完整）；`data/exam-papers.example.json` reading-writing section.questionIds 加 q-sp-002 / q-sp-003 / q-sp-004（緊接 q-sp-001 之後維持 RW3 連續分區）+ sourceMix.ai_generated 從 6 升 9 + paper-level description 從「10 題」改「13 題混合題型，含聽力、看字選圖、yes/no、看圖拼字（4 題 RW3 多元主題）」。schema 對應表 spelling 條目同步「已支援 4 題多元主題」；DATA_SCHEMA 範例 metadata 表升級為 13 題。**未動** UI 邏輯（`<SpellingView>` / `isCorrect` / `normalize` / `RW_TYPE_ORDER` / Listening / L3 / RW1 全保留）/ schema / 圖片素材（cat.svg / dog.svg / book.svg 重用、未產生新圖）/ Listening 行為。**仍未做**：缺字提示版 / 看答案按鈕 / review 區獨立練習模式整合。**硬邊界不變**。
