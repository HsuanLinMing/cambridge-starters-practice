...
0. **source registry 登錄與審核（P3-10-L，2026-05-14；P3-10-M build CLI 已落地，2026-05-15；P3-10-N collector / normalizer gate 已落地，2026-05-15；P3-10-O merge / preserve 已落地，2026-05-15）**：discovery 找到候選 URL 後，跑 `scripts/build_source_registry.mjs --input data/imported/discovered-resources.generated.json --out data/imported/source-registry.generated.json --mode build` 自動產生 source registry generated entries（**全部 pending_review / needs_manual_check，絕不 approved_for_import**）；人工審核授權 / 發行機構 / 內容 → 標 `approved_for_import` 才能進步驟 1+；`pending_review` / `needs_manual_check` / `rejected` 一律**不可**進 normalizer。**P3-10-O 起，build CLI 支援 `--merge-with <existing-source-registry.json>`**：discovery 重跑時自動保留 reviewer 已編輯的 `reviewStatus` / `rightsNotes` / `provenanceNotes` / `sourceKind` / `publisher` / `publisherType` / `partsCovered` / `level` / `exam` / `accessType` / `collectedAt` / `lastCheckedAt` / `sourceId` 欄位；既有條目若不在新 discovery input 中則保留為 orphan（不刪、不降級）；mergeKey 規則：normalize(sourceUrl) primary + title|raw-url fallback。**P3-10-N 起，下游 collector / normalizer / single-URL collector 三個 CLI 都新增 `--source-registry <path>` flag**：提供時於程式層強制 gate（exact URL normalize match，**不做** domain-level 放行）；未提供時印 warning + 保留 legacy / dev flow 行為。`source-registry.generated.json` 已 `.gitignore` 排除、**不**commit。詳見 [`docs/SOURCE_REGISTRY_PLAN.md`](./SOURCE_REGISTRY_PLAN.md) E-bis 段（含 E-bis-5 P3-10-N gate / E-bis-6 P3-10-O merge）。
...
6. **轉成 formal practice data（P3-10-K v0.1.2，2026-07-07）**：跑 `scripts/approve_reviewed_questions.mjs` 把 validate-reviewed `validationStatus=passed` 且 reviewer 已勾選 `approved=true` + `approvedForPractice=true` + `reviewStatus=approved_for_practice` 的條目扁平化為正式 `ExamQuestion`。**預設 preview**（`--mode preview`），不動正式題庫；要寫入需**雙開關**（`--mode write` + `--write yes`）；**duplicate id 全域 exit 2**——v0.1.1 拆兩種偵測：`duplicate_id_in_target`（target 既有同 id）+ `duplicate_id_in_batch`（同批 ready items 內部 id 重複），任一觸發 write mode 整批拒絕；支援 7 種題型（spelling / true-false / multiple-choice / picture-choice / word-choice / listening-choice / fill-blank），matching / listening-image-choice 標 `unsupported_question_type` skip。**`finalQuestion.source` 必須對齊 `QuestionSource` union**（`official_sample` / `past_paper` / `ai_generated` / `custom`），空值預設 `custom`，非 union 值 → `status=failed` + error `invalid_question_source`（**不** silent fallback 為 custom）；reviewer 若想表達 `user_provided` / `third_party` 等第三方來源，請保留於 `reviewerNotes` 或 discovery provenance、**不**寫入正式 `QuestionSource`。**v0.1.2 起，若 approved item 有 `sourceUrl`，正式題目會保留 optional `sourceProvenance`**（sourceId / sourceUrl / documentTitle / pageHint / sectionHint / sourceKind / publisher / publisherType / rightsNotes / provenanceNotes / reviewerNotes），讓正式題庫可回溯來源；這不代表授權或題目審核自動通過。詳見 [`docs/QUESTION_IMPORT_NORMALIZATION_PLAN.md`](./QUESTION_IMPORT_NORMALIZATION_PLAN.md) F-pre-8 段。**reviewer 跑 write 後仍須自行 git diff 確認後再 commit**——CLI 不自動 commit。
...
| `sourceProvenance` | 建議（source-first 匯入題） | 正式題目的 optional 來源追溯物件；若存在須含 `sourceUrl`，可含 `sourceId` / `documentTitle` / `pageHint` / `sectionHint` / `sourceKind` / `publisher` / `publisherType` / `rightsNotes` / `provenanceNotes` / `reviewerNotes`。只表示追溯與 reviewer 註記，不代表授權或可複製官方素材 |
| `sourceUrl` | imported workflow 欄位 | normalizer / reviewed item 的上游來源 URL；正式 `ExamQuestion` 以 `sourceProvenance.sourceUrl` 承接，不建議直接扁平寫入 |
| `sourceName` | imported workflow 欄位 | 來源可讀名稱（例如 "User-provided RW3 worksheet"）；正式題目可轉為 `sourceProvenance.documentTitle` |
| `sourceType` | imported workflow 欄位 | 對齊本檔 B 段 7 種字面量；正式 `QuestionSource` 仍只有 4 種，細分類保留於 `sourceProvenance.sourceKind` |
| `provenance` | imported workflow 欄位 | `{ resourceId, sourceDocumentId, normalizerVersion, originalText, candidateConfidence, notes }`；正式題目只保留必要追溯摘要，不複製受保護原文 |
...
- **v1.6**（2026-07-07）：對應 P3-10-V——C 段第 6 步升為 `approve_reviewed_questions.mjs` v0.1.2，補正式 `ExamQuestion.sourceProvenance` 轉換保留；D 段正式資料欄位新增 `sourceProvenance`，並把 `sourceUrl` / `sourceName` / `sourceType` / `provenance` 標為 imported workflow 上游欄位。本輪只補來源追溯 schema 與 approve preview/write 轉換，不寫正式題庫、不切 `/quiz`。
