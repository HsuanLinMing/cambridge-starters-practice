...
4. **AI 整理 / 題型分類**：normalizer（屬 P3-10-E 範圍，本輪不實作）讀 source-document → 依 candidate 推斷 `type` / `starterPart` → 出 `normalized-questions.generated.json` 草稿；reviewStatus 升為 `ai_normalized`。
5. **人工檢查**：維護者瀏覽 normalized JSON → 修正 prompt / answer / explanation / 對齊自家 SVG → 補 metadata；reviewStatus 升為 `human_review_required` 或直接到 `approved_for_practice`；不合格的標 `rejected`。
6. **轉成 formal practice data**：把 `approved_for_practice` 的題目寫入 `data/p3-example-questions.json` 或 `data/practice-questions.json`；保留所有 source / provenance 欄位。
7. **quiz / review 使用**：UI 從 `lib/data.ts` 載入；既有 `QuizPlay.tsx` / `getStarterPartInfo` / `isCorrect` 等不需動。
