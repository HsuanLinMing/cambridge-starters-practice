...
2. **P3 考前練習與題庫（進行中）**：
   ...
   - 未來測驗區會支援**一次生成一整份完整考卷**（多題型混合），可保存未完成進度（以瀏覽器 localStorage 為主，下次進來繼續作答），並提供「繼續作答 / 離開這份考卷 / 重新測驗 / 直接交卷」四個操作；交卷後評分、錯題以紅色標示、顯示正確答案與小一友善講解。題目來源標記 `official_sample` / `past_paper` / `ai_generated` / `custom`（**P3-10-L 後新規則**：正式題庫主線僅接受 `official_sample` / `past_paper`，加上對應 source registry `approved_for_import` 的可追溯來源；`ai_generated` 僅作為**正式題庫完成後的延伸題**、不得用來補正式題庫數量、不得偽裝主線標記），第一階段**不做自動爬蟲、不下載官方圖片**，**第一版不計時、不登入、不接後端 / 雲端**。詳見 `docs/PRODUCT_SPEC.md` 的「測驗與考前練習方向」、`docs/DATA_SCHEMA.md` 的 P3 schema、`docs/SOURCE_REGISTRY_PLAN.md` 的 source-first 原則、以及 `source_materials/README.md`。
...
- `docs/SOURCE_REGISTRY_PLAN.md` — 正式來源優先匯入規則 + Source Registry（P3-10-L）：正式匯入版只接受可追溯來源；`ai_generated` 不得補正式題庫數量；E-bis 段：discovery → source registry generated workflow（`scripts/build_source_registry.mjs` v0.1，P3-10-M）
