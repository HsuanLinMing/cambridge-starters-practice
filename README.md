...
- `/quiz`：**測驗區 P3-6-A + P3-6-B-1 / P3-6-B-2 + P3-9-B 第一刀**——載入 P3-1 範例考卷（`data/exam-papers.example.json` + `data/p3-example-questions.json`），題目排序貼近正式 Cambridge Starters：**Section 1 Listening（`listening-choice`）→ Section 2 Reading & Writing（`picture-choice` → `word-choice` → `multiple-choice` → `fill-blank` → `matching`）**；題目卡頂端三列：段落徽章（「Section 1 · Listening｜聽力練習」sky 配色 / 「Section 2 · Reading & Writing｜閱讀與書寫練習」amber 配色）+ **Part 標示行**——範例題目已補 `starterSection` / `starterPart` / `skillFocus` / `expectedAnswerType` metadata（P3-9-B 第一刀），`/quiz` Part 顯示**優先讀題目 metadata**（缺值時 fallback 依題型推導）；7 題對應為 q-lc-001 → L3 聽音選圖 / q-pc-001 → RW1 看圖判斷 / 看圖選答案 / q-wc-001 → RW3 看圖認字 / 拼字練習 / q-mc-001 → RW4 preview 短句選字 / 詞彙選擇 / q-fb-001 + q-fb-002 → RW4 短文 / 句子填空 / q-mt-001 → RW5 圖文配對 / 故事理解預備（**目前仍是練習版近似對應**，未來會依 P3-7-B / P3-9-C 校正）；**Part 顯示已改為 metadata-first，並可依題型微調練習版文案**——同一 `starterPart` 下不同 `question.type` 顯示更精準的中文題型描述（例如 RW4 + fill-blank 顯示「短文 / 句子填空」、RW4 + multiple-choice 顯示「短句選字 / 詞彙選擇 preview」）；**Speaking metadata（SP1~SP4 / `"speaking"` section / `"spoken"` answer type）只是預留型別字面量、尚未實作**——留給 P4 Speaking Examiner Agent。題目卡頂端進度行 + 頁首補小字「目前為練習版，題型逐步對齊正式 Cambridge Starters」。6 題型最小渲染、一題一頁、未答題「下一題」disabled、最後一題顯示「看結果」。**作答進度自動保存於本機 localStorage**（key `cambridge-starters-practice:quiz-session:v1`，由 `lib/examSessionStorage.ts` 集中處理）——重新整理 / 重開分頁可恢復進度（同 paperId + questionOrder 才恢復、不相容自動丟棄）；恢復時畫面頂端顯示 emerald 系小提示「🔁 已恢復上次作答進度」首次點選後消失。題目卡下方加兩個 chip 按鈕：**「📝 直接交卷」**（提前進結果頁，未作答題算錯）+ **「🔁 重新測驗」**（清 localStorage、回第一題、無 confirm dialog）。**結果頁**顯示「答對 N / 共 M 題」+「已作答 X / 共 M」+「未作答 M-X 題」雙欄統計（emerald / rose 配色）+ 鼓勵文案 + 重新測驗，並在下方加 **「每題詳解」清單**（每題一張卡片，emerald 答對 / rose 答錯 / amber 未作答三色配色，每張卡片顯示：第幾題 + Section + Part 標示 + 狀態 chip + 題目文字版 + 你的答案 + 正確答案 + 說明；matching 顯示「已完成閱讀配對練習」、未作答顯示「尚未作答」並不算對；無 `explanation` 時依狀態顯示鼓勵性 fallback）。詳解清單上方加 **inline 篩選** 4 個 chip：「全部 N」/「只看錯題 X」/「只看未作答 Y」/「需要再練習 Z」（review = 錯題 + 未作答；matching 完成後算對、不出現在 review）；每個 chip 內嵌數字 badge；篩選結果為空時顯示 emerald 系友善提示（如「太棒了，目前沒有答錯的題目！」/「全部都很棒，這次沒有需要再練習的題目！」）；篩選狀態純 React local state、不存 localStorage、重整後回到預設「全部」。當「需要再練習」數量 > 0 時，結果頁顯示 rose-400 系按鈕「🔁 再練習這些題目（N）」進入 **inline 再練習模式**：amber 系 banner 提示「再練習結果不會覆蓋原始測驗分數」+ 進度顯示「再練習 第 X / Y 題」（retry-relative，Y = 錯題 + 未作答數）+ 完成後顯示「本次再練習結果」（含 retry 統計 / 鼓勵語 / retry 每題詳解 / 「↩ 回到完整測驗結果」/「🔁 重新測驗（清除原始與再練習進度）」）；retry mode 純 React in-memory state，**不**新增路由、**不**存 localStorage、**不**升 schemaVersion、**不**覆蓋原始測驗分數，重整頁面後因 localStorage 仍是 submitted=true 的原始 session、會自動回到完整結果頁。圖片缺檔 fallback（首字母 + 「圖片準備中」）；**Listening 題型已支援自製音檔欄位 + 第一個自製 TTS 音檔已產生**（P3-9-C 第一刀 + P2-4C-2B-2）——`listening-choice` 補 optional `audioSrc` 欄位，UI 在音檔可載入時 render `<audio controls>` + 🎧「請先聽音檔，再選答案」+「文字稿會在交卷後訂正時顯示」+「💡 正式考試中錄音會播放兩次；本練習版可自行重播音檔練習」三段提示；**第一個自製音檔 `q-lc-001.m4a`** 已透過 macOS `say` + `afconvert` 流程產生於 `public/audio/starters/l3/`（AAC m4a 格式，~12 KB / 1.86 秒），完整流程記錄於 [`docs/TTS_AUDIO_WORKFLOW.md`](./docs/TTS_AUDIO_WORKFLOW.md)（第一階段只用本機流程）。**目前正在評估 OpenAI TTS examiner voice**——`scripts/generate_openai_tts_sample.mjs` 一題試產腳本，與 macOS `say` 版本（`q-lc-001.m4a`）並存供使用者實聽比較。**v1**（`q-lc-001-openai.mp3`）使用者回饋「比 macOS say 更正式、有 examiner 感、但語速太慢、有點刻意」；**v2**（預設輸出 `q-lc-001-openai-v2.mp3`）已調整 instructions 為「natural exam pace, not overly slow」+「Do not over-emphasize each word」+ 把 British 改 standard，**待使用者實聽**；可用環境變數 `OPENAI_TTS_OUTPUT_SUFFIX=v3` 等產生更多版本。OpenAI TTS **只用於把自製文字轉自製音檔，絕不上傳官方題目原文 / 歷屆題給 OpenAI**；API key 透過 `.env.local`（已被 `.gitignore` 排除）讀取，**不會 commit**；**TTS voice 是 AI-generated，不是真人考官聲音**——使用時 UI / 文件處處標示。本輪 Claude 未實際呼叫 API（環境無 API key），需家長 / 維護者本機設定後自行跑；**`/quiz` 仍使用 macOS say 版本** `q-lc-001.m4a`，切換 audioSrc 屬下一輪人工實聽確認後動作。詳細實聽紀錄與 v3+ 觸發條件見 [`docs/TTS_AUDIO_WORKFLOW.md`](./docs/TTS_AUDIO_WORKFLOW.md)「OpenAI examiner voice 實聽調整紀錄」段。**Listening 題在正式練習時隱藏文字稿**（P3-9-C 第二刀，2026-05-10）——避免孩子直接看到題目原文；**交卷後結果頁詳解仍顯示 transcript** 供家長 / 孩子訂正；**音檔缺失（路徑無檔 / 載入失敗）時才 fallback 顯示「音檔準備中，先用文字練習」+ transcript 文字練習**。**不使用官方音檔**——音檔需自製或由 TTS 自製（屬 P2-4C-2B-2）；**Speaking 不做**。**獨立錯題複習頁** / `/quiz/wrong` 路由 / `wrongQuestionIds` 持久化 / **錯題歷史紀錄** / **計時器** 屬 P3-6-B-4 後續刀數 / P3-6-B-5，本輪未做
