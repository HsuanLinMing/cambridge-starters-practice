...
- ⬜ 使用者實聽比較 macOS `say` vs OpenAI TTS（家長 / 維護者本機設定 OPENAI_API_KEY 後跑 `node --env-file=.env.local scripts/generate_openai_tts_sample.mjs` 產生 `q-lc-001-openai.mp3`，然後 `afplay` 或瀏覽器播放對比 `q-lc-001.m4a` 是否更接近正式考試員音色）
