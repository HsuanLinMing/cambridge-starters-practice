...
- 🟡 使用者實聽比較 macOS `say` vs OpenAI TTS——**v1 已實聽**（家長 2026-05-10 回饋：比 macOS say 正式但語速太慢、有點刻意）；**v2 instructions 已調整**（natural exam pace + not overly slow + Do not over-emphasize each word + British 改 standard）；**v2 待實聽**——使用者本機跑 `node --env-file=.env.local scripts/generate_openai_tts_sample.mjs` 產生 `q-lc-001-openai-v2.mp3` 後實聽比較 v1 / v2 / macOS say 三版
...
- ⬜ 若實聽通過，切換 `q-lc-001` 的 `audioSrc` 從 `.m4a` 改為 `.mp3`（屬下一輪獨立任務；保留 macOS `say` 版本不刪、兩版並存供日後比較）
